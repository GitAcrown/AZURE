...
async def main() -> None:
    ...
    logger.info("Chargement du catalogue AZURE...")
    try:
        bot.catalog = load_catalog(Path("assets"))  # type: ignore[attr-defined]
    ...
    if api_key:
        ...
        bot.llm = LLMClient(api_key)  # type: ignore[attr-defined]
    else:
        bot.llm = None  # type: ignore[attr-defined]
        logger.warning("OPENAI_API_KEY absente — le dialogue village sera indisponible.")

    logger.info("Ouverture de la base joueur...")
    bot.store = await open_store(Path("data/azure.db"), bot.catalog)  # type: ignore[attr-defined]
    ...
    async with bot:
        ...
        async def on_app_command_error(
            ...
            if isinstance(error, app_commands.NoPrivateMessage):
                await interaction.response.send_message(
                    "Cette commande s'utilise sur un serveur.",
                    ephemeral=True,
                )
                return
            if isinstance(error, app_commands.CheckFailure):
                await interaction.response.send_message(
                    "**Erreur ·** Cette commande est réservée au propriétaire du bot.",
                    ephemeral=True,
                )
                return
